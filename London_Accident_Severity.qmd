...
Word count (narrative only): \~1,598 words
...
| `time_period_enc` | Categorical | AM peak / inter-peak / PM peak / night; captures systematic temporal severity patterns. |
| `year` | Integer | Collision year (2020–2024); captures long-term drift and structural changes in road safety. |
| `quarter` | Categorical | Calendar quarter (1–4); groups Jan-Mar, Apr-Jun, Jul-Sep, Oct-Dec to capture seasonal risk profiles. |
...
| `mean_driver_imd` | Ordinal | Mean IMD decile of drivers' home postcodes per collision; proxy for vehicle quality and risk behaviour. |
| `mean_casualty_imd` | Ordinal | Mean IMD decile of casualties' home postcodes per collision; proxy for individual vulnerability. |
...
1.  From 2020 to 2024, **KSI proportion rose from 14.2% to 17.4%**, with 2020 likely reflecting pandemic-suppressed volumes rather than higher safety (Figure 1A).
...
4.  P**edestrians faced the highest conditional risk at 276 KSI per 1,000 collisions**, nearly three times that of motorcyclists (Figure 1E).
